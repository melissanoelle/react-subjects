...
describe('when <Tabs> is rendered', () => {
  ...
  it('activates the first tab', () => {
    console.log(tabs)
    expect(tabs[0].style.borderBottomColor).toEqual(borderFixture.style.borderBottomColor)
  })

  it('does not activate the second tab', () => {
    expect(tabs[1].style.borderBottomColor).toNotEqual(borderFixture.style.borderBottomColor)
  })

  it('puts the correct content in the panel', () => {
    expect(panel.innerHTML).toEqual(FixtureData[0].content, 'Correct content was not shown for first tab.')
  })
  ...
  describe('after clicking the second tab', () => {
    ...
    it('activates the second tab', () => {
      expect(tabs[1].style.borderBottomColor).toEqual(borderFixture.style.borderBottomColor)
    })

    it('deactivates the first tab', () => {
      expect(tabs[0].style.borderBottomColor).toNotEqual(borderFixture.style.borderBottomColor)
    })

    it('puts the correct content in the panel', () => {
      expect(panel.innerHTML).toEqual(FixtureData[1].content, 'Correct content was not shown for second tab.')
    })
  })
})
